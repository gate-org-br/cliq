...
public class AtendimentoReport extends CliQReport
{

	public AtendimentoReport(Doc.Type type, Org org, List<Chamado> chamados)
	{
		...
		if (!chamados.isEmpty())
		{
			...
			if (type == Doc.Type.XLS || type == Doc.Type.CSV)
			{
				...
				grid.add().head("Prioridade").body(Chamado::getPrioridade);
				grid.add().head("Complexidade").body(Chamado::getComplexidade);
				grid.add().head("Resposta").body(Chamado::getResposta);
				grid.add().head("Solução").body(Chamado::getSolucao);

				grid.add().head("Status").body(e -> e.getEvento().getStatus());
				grid.add().head("Observações").body(e -> XMLCleaner.cleanup(e.getEvento().getObservacoes()));
			} else
			{
